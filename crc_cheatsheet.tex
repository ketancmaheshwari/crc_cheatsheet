...
\begin{itemize}\compresslist
\item Chemistry: $\text{NAMD}^{s}$, casino, lammps, $\text{Amber}^{g}$, {\color{red}$\text{Molpro}^{s}$}, {\color{red}$\text{Turbomole}^{s}$}, $\text{CP2K}^{s}$, {\color{red}$\text{Gaussian}^{s}$}
\item Bio: $\text{CLCbio}^{h}$, $\text{galaxy}^{h}$, $\text{bowtie}^{h}$, $\text{samtools}^{h}$, $\text{picard}^{h}$, $\text{trinity}^{h}$
\item Material Sci: {\color{red}Material Studio}, Westpa, {\color{red}abacus}, {\color{red}$\text{VASP}^{s}$}
\end{itemize}

%----Libraries and Programming-----------
\colorbox[HTML]{cdb87d}{\makebox[\textwidth-2\fboxsep][l]{\bf - Libraries and Programming}}
\begin{itemize}\compresslist
\item Compilers: GCC, Intel
\item Programming: Python, R, Java, Matlab
\item Libraries: Boost, FFT, $\text{tensorflow}^{h,g}$, mkl, hdf5, $\text{bioconductor}^{h}$
\end{itemize}

%-------Others---------------------
\colorbox[HTML]{cdb87d}{\makebox[\textwidth-2\fboxsep][l]{\bf - Others}}
\begin{itemize}\compresslist
\item Editors: vim, emacs, nano
\item Debuggers: gdb, gprof
\item Shells: bash, zsh
\end{itemize}
}
...
\begin{itemize}\compresslist
\item 24 nodes of 12-core Xeon E5-2643v4 3.40 GHz (Broadwell), 256 G RAM, 256 G SSD \& 1 T SSD, 10GigE
\item 2 nodes of 12-core Xeon E5-2643v4 3.40 GHz (Broadwell), 256 G RAM, 256 G SSD \& 3 T SSD, 10GigE
...
\#SBATCH --job-name=abf5\\
\#SBATCH --output=abf5.out\\
