...
SMP/OPA/NTA \texttt{h2p.crc.pitt.edu}\\ 
\texttt{login0b.mpi.sam.pitt.edu}
%--------------------------------------
\colorbox[HTML]{cdb87d}{\makebox[\textwidth-2\fboxsep][l]{\bf - Storage}} 
%Storage facilities
ihome  \texttt{/ihome/grp} (backed up) \\
mobydisk  \texttt{/mnt/mobydisk} (not backed up) \\
ZFS  \texttt{/zfs1 \& /zfs2} (not backed up)
...
Ketan Maheshwari: GPU Computing, Scientific Workflows, HPC  \\
...
\item Chemistry: $\text{NAMD}^{s}$, {\color{red}$\text{VASP}^{s}$}, $\text{Amber}^{g}$, {\color{red}$\text{Molpro}^{s}$}, {\color{red}$\text{Turbomole}^{s}$}, $\text{CP2K}^{s}$, {\color{red}$\text{Gaussian}^{s}$}
\item Bio: $\text{CLCbio}^{h}$, $\text{galaxy}^{h}$
\item Material Sci: {\color{red}Material Studio}, Westpa
\end{itemize}
...
\item Libraries: Boost, FFT, $\text{tensorflow}^{h,g}$, mkl, hdf5
\end{itemize}
