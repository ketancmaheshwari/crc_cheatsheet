...
%	TITLE
%----------------------------------------------------------------
{\bf\textsc{Quick Summary}\vspace{0.5em}} % Poster title
...
\texttt{ssh h2p.crc.pitt.edu \#connect to the SMP, OPA and GPU clusters }\\
\texttt{ssh htc.sam.pitt.edu \#connect to the HTC cluster }\\
%------
\colorbox[HTML]{cdb87d}{\makebox[\textwidth-2\fboxsep][l]{\bf - Data}}
\texttt{scp afile.tgz htc.sam.pitt.edu:\textasciitilde/  \#copy from local computer to smp cluster }\\
\texttt{scp htc.sam.pitt.edu:\textasciitilde/afile.tgz .  \#copy from htc to local computer} \\
\texttt{rsync -aP \$HOME/src /zfs1/1/sam/ketan  \#copy src to zfs using rsync}\\
\texttt{df -h /zfs1/2/kjordan/ \#find quota of a group's ZFS storage}\\
\texttt{lfs quota -g jpipas /mnt/mobydisk \#find quota of a group's mobydisk storage}\\
\linebreak
\colorbox[HTML]{cdb87d}{\makebox[\textwidth-2\fboxsep][l]{\bf - LMod: load and unload software}}
\texttt{module spider fftw  \#search for module named fftw} \\
\texttt{module avail  \#list available modules} \\
\texttt{module list   \#list currently loaded modules} \\
\texttt{module load compiler/python/2.7.10-Anaconda-2.3.0   \#load anaconda module}\\
\texttt{module purge  \#unload all modules }
%------
}
...
\colorbox[HTML]{cdb87d}{\makebox[\textwidth-2\fboxsep][l]{\bf - SMP$_{Standard}$ 10 GIGE NETWORK}}
\begin{itemize}\compresslist
\item 100 nodes of 24-core Xeon Gold 6126 2.60 GHz (Skylake), 192 G RAM, 256 G \& 500 G SSD 
\item 24 nodes of 12-core Xeon E5-2643v4 3.40 GHz (Broadwell), 256 G RAM, 256 G SSD \& 1 T SSD
\end{itemize}

\colorbox[HTML]{cdb87d}{\makebox[\textwidth-2\fboxsep][l]{\bf - SMP$_{Specialty}$}}
\begin{itemize}\compresslist
\item 2 nodes of 12-core Xeon E5-2643v4 3.40 GHz (Broadwell), 256 G RAM, 256 G SSD \& 3 T SSD, 10 GigE
\item 2 nodes of 12-core Xeon E5-2643v4 3.40 GHz (Broadwell), 512 G RAM, 256 G SSD \& 3 T SSD, FDR-IB
\item 1 node of 12-core Xeon E5-2643v4 3.40 GHz (Broadwell), 256 G RAM, 256 G SSD \& 6 T NVMe, GigE
\end{itemize}

\colorbox[HTML]{cdb87d}{\makebox[\textwidth-2\fboxsep][l]{\bf - MPI$_{OP}$ OMNI-PATH NETWORK}}
\begin{itemize}\compresslist
%\item (IB) 32 nodes of 20-core Haswell (E5-2660 v3), 2.6 GHz (Haswell), 128 G RAM, 256 G SSD, FDR InfiniBand
\item 96 nodes of 28-core Intel Xeon E5-2690 2.60 GHz (Broadwell), 64 G RAM, 256 G SSD, 100 Gb
\end{itemize}

\colorbox[HTML]{cdb87d}{\makebox[\textwidth-2\fboxsep][l]{\bf - MPI$_{IB}$ INFINIBAND NETWORK}}
\begin{itemize}\compresslist
\item (IB) 32 nodes of 20-core Haswell (E5-2660 v3), 2.6 GHz (Haswell), 128 G RAM, 256 G SSD
%\item 96 nodes of 28-core Intel Xeon E5-2690 2.60 GHz (Broadwell), 64 G RAM, 256 G SSD, 100 Gb Omni-Path
\end{itemize}

\colorbox[HTML]{cdb87d}{\makebox[\textwidth-2\fboxsep][l]{\bf - HTC INFINIBAND NETWORK}}
\begin{itemize}\compresslist
\item 4 nodes of 24-core Xeon Gold 6126 2.60 GHz (Skylake) 384 G RAM, 256 G \& 500 G SSD
\item 20 nodes of 16-core Intel Xeon E5-2630v3, 2.4GHz (Haswell-EP), 256 G RAM, 256 G SSD
\end{itemize}
...
\headerbox{Job Scheduler Policy}{name=method,column=3, below=help}{
\textbf{Details coming soon.}
\colorbox[HTML]{cdb87d}{\makebox[\textwidth-2\fboxsep][l]{\bf - Charging}}
Users are charged in terms of Service Units (SU) which depend on both memory and CPU usage.
\colorbox[HTML]{cdb87d}{\makebox[\textwidth-2\fboxsep][l]{\bf - Queues and limits}}
-- Minimum and maximum walltimes\\
-- Minimum and maximum nodes that may be requested\\
}
...
\linebreak
Below is the command to see details of the nodes on a particular cluster. The key column is the CPUS(A/I/O/T) where,\\
A = allocated, I = idle, O = other, T = total\\
Single-core jobs should fit within nodes with non-zero I.\\
\texttt{sinfo -{}-format="\%10n \%9P \%.14C \%.10T"}
\linebreak
\textbf{Note:} The above commands will show info about the \emph{SMP} cluster by default. To see info about other clusters, use the \texttt{\textbf{-M}} option with name of cluster, eg.\texttt{squeue \textbf{-M gpu} -u \$(whoami)}\\ 
%\texttt{squeue -j 12345 -o \%S}\hfill \#expected or actual start time of job id 12345\\
\linebreak
\texttt{crc-sinfo.py}\hfill \#current hardware status\\
\texttt{crc-spider.py}\hfill \#regular expression search for modules\\
\texttt{crc-squeue.py -{}-start}\hfill \#show job start time\\
\texttt{crc-squeue.py -{}-watch}\hfill \#watch job progress\\
\texttt{crc-squeue.py -{}-all}\hfill \#show all jobs\\
\texttt{crc-usage.py}\hfill \#show your usage on cluster\\
\texttt{crc-interactive.py -{}-smp -{}-time=1 -{}-num-cpus=2}\hfill \#run interactive job on cluster
...
\#!/bin/bash\\
\#SBATCH -N 1 \hfill\# request one node, ensure that all cores are on one machine\\
\#SBATCH -{}-job-name=testR \hfill\#name of job: will show up in status output of \texttt{squeue}\\
\#SBATCH -{}-output=testR.out \hfill\#standard output goes to this file, SLURM will create one if not provided\\
\#SBATCH -t 00:10:00 \hfill\# 10 minutes walltime in HH:MM:SS format\\
...
\#SBATCH -{}-cluster=mpi \hfill\# cluster name is required \\
...
\headerbox{Notes}{name=notes, below=troubleshoot, column=3,row=1}{
\begin{itemize}\compresslist
